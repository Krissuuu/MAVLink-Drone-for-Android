...
public class Drone_Message {
    ...
    static String mav_type = null, mav_autopilot = null, system_status = null, flight_mode = null, arm_disarm = null;
//    static Character arm_disarm = null;
    ...
    public static void Message_classify(MavlinkMessage message){
        // HEARTBEAT ( #0 )
        if (message.getPayload() instanceof Heartbeat) {
            ...
            if(mav_type_.equals("MAV_TYPE_QUADROTOR"))
            {
                ...
                String base_mode_msg = Integer.toBinaryString(Integer.valueOf(base_mode));
                if(base_mode_msg.length() == 7){
                    base_mode_msg = "0"+base_mode_msg;
                }
                else if(base_mode_msg.length() == 6){
                    base_mode_msg = "00"+base_mode_msg;
                }
                else if(base_mode_msg.length() == 5){
                    base_mode_msg = "000"+base_mode_msg;
                }
                else if(base_mode_msg.length() == 4){
                    base_mode_msg = "0000"+base_mode_msg;
                }
                else if(base_mode_msg.length() == 3){
                    base_mode_msg = "00000"+base_mode_msg;
                }
                else if(base_mode_msg.length() == 2){
                    base_mode_msg = "000000"+base_mode_msg;
                }
                else if(base_mode_msg.length() == 1){
                    base_mode_msg = "0000000"+base_mode_msg;
                }
                arm_disarm = String.valueOf(base_mode_msg.charAt(0));

//                Log.i("Drone_Message", payload);
                Log.i("Drone_Message", "HEARTBEAT: " + mav_type + " " + mav_autopilot + " " +
                                                    flight_mode + " " + system_status + " " + arm_disarm);
            }
        }
...
//            Log.i("Drone_Message", payload);
            Log.i("Drone_Message", "GLOBAL_POSITION_INT: " + lat + " " + lng + " " + alt + " " +
                                                                        relative_alt + " " + heading);
        }
        // SYS_STATUS ( #1 )
...
//            Log.i("Drone_Message", payload);
        ...
        // COMMAND_ACK ( #77 )
            ...
            what_cmd = payload_CommandAck[1];
            cmd_result = payload_CommandAck[3];

            JSONObject cmd_ack = new JSONObject();
            try {
                cmd_ack.put("device_id", Phone_Message.device_id);
            ...
            MQTT.MQTT_PUBLISH_CMD_ACK(cmd_ack.toString());
        ...
        // STATUSTEXT ( #256 )
            ...
            status_txt = payload_Statustext[2].trim();

            JSONObject cmd_apm_text = new JSONObject();
            try {
                cmd_apm_text.put("device_id", Phone_Message.device_id);
                cmd_apm_text.put("text", status_txt);
            }
            catch (JSONException e) {
                e.printStackTrace();
            }
            MQTT.MQTT_PUBLISH_APM_TEXT(cmd_apm_text.toString());
        ...
        // MISSION_ACK ( #47 )
            ...
            mission_result = payload_MissionAck[3].trim();

            JSONObject mission_ack = new JSONObject();
            try {
                mission_ack.put("device_id", Phone_Message.device_id);
                mission_ack.put("mission_result", mission_result);
            }
            catch (JSONException e) {
                e.printStackTrace();
            }
            MQTT.MQTT_PUBLISH_MISSION_ACK(mission_ack.toString());
    ...
    public static JSONObject PacketToJson(){
        ...
        try {
            ...
            heartbeat.put("is_armed", arm_disarm);
    ...
    public static String MODE_TYPE(String num){
        switch (num){
            case "0":
                return "STABILIZE";
            case "1":
